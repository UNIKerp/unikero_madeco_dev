...
class IrModuleModule(models.Model):
    ...
    def _get_modules_state(self):
        ...
        imported_modules = self.search(
            [('name', 'in', modules_availabilty['imported_modules'])])
    ...
    def get_overall_update_state(self):
        ...
        * states:
        ...
        imported_modules = modules_state.get('imported_modules')
        ...
        return {
            ...
            'detail': {
                ...
                'imported_modules': imported_modules.mapped('name'),
            }
        }
    ...
    def get_modules_availabilty(self):
        ...
        base_import_module = self.search([
            ('name', '=', 'base_import_module'),
            ('state', '=', 'installed')])
        modules_list = self.search(
            [('state', 'in', states)] +
            (base_import_module and [('imported', '=', False)] or [])
        ).mapped('name')
        ...
        unmet_deps = list(filter(lambda x: x not in modules_list, unmet_deps))

        # Imported modules
        imported_modules = self.search([
            ('state', 'in', states), ('imported', '=', True)]).mapped('name') \
            if base_import_module else []
