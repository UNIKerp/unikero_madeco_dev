...
class ImportEncodage(models.Model):
    _name = "import.encodage"
    _description = "Import Encodage"

    name = fields.Char("Nom")
    code = fields.Char("Code")


class ImportTypeSource(models.Model):
    _name = "import.type_source"
    _description = "Import Type Source"
    ...
    encodage = fields.Many2one(
        "import.encodage", string="Encodage", default=get_default_encodage
    )
...
class ImportSource(models.Model):
    ...
    name = fields.Char("Nom", required=True)
    ttype = fields.Many2one(
        "import.type_source",
        string="Type Source",
        required=True,
        default=get_default_ttype,
    )
    ...
    encodage = fields.Many2one(
        "import.encodage", string="Encodage", related="ttype.encodage"
    )
        ...
        for i in self:
            ...
            if (
                ...
                if len(data) > 0:
                    ...
                    _logger.info("erreur, aucun objet trouvé")
    ...
    def charger_donnees(self):  # noqa: C901
        for i in self:
            if i.chargeable:
...
class ImportData(models.Model):
    ...
    donnees_chargees = fields.Boolean(
        "Données chargées", related="source.donnees_chargees"
    )
    ...
    def lancer_moulinette(self, records=None):
        ...
        if not self.par_bloc:
            try:
                ...
                datas = obj.search(  # noqa: F841
                    eval(self.domain), limit=self.limit, offset=self.offset
                )
            ...
            try:
                ...
                for o in range(0, count, self.taille_bloc):
                    datas = obj.search(  # noqa: F841
                        eval(self.domain), limit=self.taille_bloc, offset=o
                    )
                    exec(self.moulinette)
                    self.env.cr.commit()
                    _logger.info(o)
