...
class wizard_pdp_export(models.TransientModel):
	...
	def _get_production_schedule(self):
		return self.env['mrp.production.schedule'].search([])
	
	
	production_schedule_ids = fields.Many2many('mrp.production.schedule',string='Produits',default=_get_production_schedule)
	...
	def exporter(self):
		...
		for production_schedule_id in self.production_schedule_ids:
			...
			line = [[ '', ],['{}-{}'.format(production_schedule_id.product_id.id,production_schedule_id.product_id.name),],['Demande planifiée'],['Réappro suggéré'],['Stock planifié'],['Entrepot ID']]
