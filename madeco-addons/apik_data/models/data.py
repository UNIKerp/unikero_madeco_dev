...
class apik_data(models.Model):
    ...
    def executer_maj(self):
        ...
        for r in self:
            if self.env.user.apik_data_select:
                ...
                import psycopg2
                conn = psycopg2.connect(host=host,user=self.env.user.company_id.apik_data_user,password=self.env.user.company_id.apik_data_password, database=base,port=port)
    ...
    def executer(self):
        ...
        for r in self:
            ...
            historique = historique_obj.search([('name','=',r.requete),('requete','=',r.id)])
            if len(historique)==0:
                value = {
                    'name': r.requete,
                    'requete': r.id
    ...
    def creer_vue(self):
        ...
        """
        if '*' in self.requete:
            raise UserError("Attention, vous ne pouvez utiliser le * dans une requête pivot !")
        if self.requete[:6].upper() != "SELECT":
            raise UserError('Attention, la requête doit commencer par SELECT dans une requête pivot !')
        """
        ...
        sql = "CREATE OR REPLACE view {} AS SELECT * from ({}) as mon_modele".format(name_model,self.requete)
        
        self.env.cr.execute(sql)
        sql_type_column = "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '{}' ORDER  BY ordinal_position;".format(name_model)
        ...
        if len(action) == 0:
            ...
            logger.info(value_action)
            action = action_obj.create(value_action)

            arch_base = '<?xml version="1.0"?><pivot disable_linking="{}" string="{}">'.format(self.disable_linking,name_model)
            for f in columns:
        ...
        logger.info("drop table : {}".format(sql))
        self.env.cr.execute(sql)
        
        sql = "SELECT CAST(row_number() OVER () as integer) AS id,CAST(Null as timestamp without time zone) as create_date,CAST(Null as integer) as create_uid, CAST(Null as timestamp without time zone) as write_date, CAST(Null as integer) as write_uid,{} from ({}) as mon_modele".format(",".join(['"{}" as {}'.format(column['column_name'],'x_{}'.format(column['column_name'])) for column in columns]),self.requete)
