...
class ArcCommandeEDIExport(models.TransientModel):
    ...
    ################################
    @api.model
    def export_arc_cde_edi_automatique(self):
        wizard_values = {
            'export_auto': True,
        }
        ...
        logger.info("=====================================================")
        logger.info("     Début Export des ARC de commandes EDI    ")
        logger.info("=====================================================")                
        wizard.export_arc_cde_edi()
        logger.info("=====================================================")
        logger.info("      Fin Export des ARC de commandes EDI     ")
        logger.info("=====================================================")
        
    def export_arc_cde_edi_old(self):           
        self.ensure_one()
        today = fields.Date.to_string(datetime.now())
        company_id =  self.env.company                
        partner_obj = self.env['res.partner']
        sale_obj = self.env['sale.order']
        if self.export_auto:
             sale_ids = self.env['sale.order'].search([('state','=','sale'),('company_id','=',company_id.id),('commande_edi','=',True),('arc_edi_envoye','=',False)],limit=10)
        else:
            sale_ids = sale_obj.search([('id', 'in', self._context.get('active_ids', True))])
        nb_enreg = 1
        rows_to_write = []
                ...
                if sale.partner_id.edi_ordrsp:
                    ...
                    for line in sale.order_line:      
                        ...
                        if line.product_id.typologie_article  == 'A3':
                            enreg_lig = enreg_lig + line.unik_ref_prestashop +";"
                            print('#################################### Couple Prestashop dans les cmd EDi')
                            print(line.unik_ref_prestashop)
                            print('#################################### Couple Prestashop dans les cmd EDi')
                            ref_of = ''
                            ref_cde_achat = ''
                            if line.ordre_fabrication_id:
                                ref_of = line.ordre_fabrication_id.name 
                                print('#################################### OF dans les cmd EDi')
                                print(ref_of)
                                print('#################################### OF dans les cmd EDi')
                            elif line.bon_commande_id:
                                ref_cde_achat = line.bon_commande_id.name
                                print('#################################### HA dans les cmd EDi')
                                print(ref_of)
                                print('#################################### HA dans les cmd EDi')
                            enreg_lig = enreg_lig + ref_of + ";" + ref_cde_achat + ";"
                        if enreg_lig:
                            rows_to_write.append(enreg_lig)
                            nb_enreg = nb_enreg + 1
                else:
        ...
        if nb_enreg >= 1:
            ...
            if len(param)>0: 
                if param.type_connexion == 'ftp': 
                    ...
                    rep_envoi_ftp = param.repertoire_envoi_edi
                    print("------------------------------------------------------------")
                    print(ftp_user,ftp_password,adresse_ftp,rep_envoi_ftp)
                    print("------------------------------------------------------------")
                    ftp = ftplib.FTP() 
                    ftp.connect(adresse_ftp, ftp_port, 30*5) 
                    ftp.login(ftp_user, ftp_password)             
